...
export const Overview = () => {
  ...
  // Add safety check for undefined seat_breakdown
  const seat_breakdown = seatManagement?.seat_breakdown || { 
    total: 0, 
    active_this_cycle: 0, 
    inactive_this_cycle: 0 
  };
  
  const { total, active_this_cycle, inactive_this_cycle } = seat_breakdown;
  ...
  return (
    <Card className="col-span-1">
      ...
      <CardContent className=" flex flex-col gap-2">
        <Item label="Total seats" value={total || 'N/A'} />
        <Item label="Active seats" value={active_this_cycle || 'N/A'} />
        <Item label="Inactive seats" value={inactive_this_cycle || 'N/A'} />
      </CardContent>
    </Card>
  );
};
